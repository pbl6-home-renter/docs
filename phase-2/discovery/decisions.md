...
- **Decision:** Split by **role** and by **business action**. Contract becomes **108 endpoints in 25 groups**.
  ...
  - *Action split on mutations:* `status` is removed from the body of `PATCH /users/{userId}`, `PATCH /buildings/{buildingId}` and `PATCH /rooms/{roomId}`. Each lifecycle transition becomes its own `POST .../{business-action}`: `lock`, `unlock`, `approve`, `reject`, `mark-ready`, `cancel`.
  - *Exception — issue resolution:* `PATCH /issues/{issueId}` **keeps** `status` (`resolved` / `closed`) alongside `note`. Resolving a report is not a moderation transition with its own permission boundary — it is the landlord working a thread it already owns, under the same `landlord` check the PATCH performs. `POST /issues/{issueId}/cancel` stays a separate action because cancelling is scoped to the tenant who filed the report, so that transition does change the calling party.
- **Correction (2026-09-26):** the action split originally listed `PATCH /issues/{issueId}` together with the other three. Reconciling D48 against the implemented contract showed the split was never applied to issue resolution, and the exemption above is the intended scope. This correction adds no endpoint: the contract stays at 106 after D49.
...
## D50 - Three contract gaps closed after the D49 review

- **Date:** 2026-09-26
- **Status:** Accepted
- **Context:** Review D49 lộ ba chỗ hợp đồng không phục vụ được nghiệp vụ mà vẫn trông đầy đủ. (1) `GET /admin/buildings` (mã 24) là hàng đợi duyệt nhưng không có route chi tiết; `GET /buildings/{buildingId}` (mã 26) cố tình trả `404` cho tòa nhà `pending`/`rejected`, nên admin chỉ có thể bấm `approve` mà không đọc được nội dung cần duyệt. (2) Có `DELETE /buildings/{buildingId}` (mã 31) nhưng không có `DELETE /rooms/{roomId}`, nên chủ nhà đăng nhầm một phòng chỉ gỡ được cả tòa nhà. (3) Nhóm MatchRequest chỉ có accept/reject phía bên nhận; người gửi không có lối rút, và nếu bên nhận im lặng thì yêu cầu kẹt `pending` vĩnh viễn. Baseline route 36 từng là `DELETE /match-requests/{matchRequestId}` nhưng đã bị gỡ vì `MatchRequest.status` không có giá trị hủy.
- **Decision:** Thêm 3 endpoint, đánh số **108–110**, giữ nguyên toàn bộ mã cũ và giữ mã `32` trống.
  - `108` `GET /admin/buildings/{buildingId}` (`admin`) — chi tiết tòa nhà cho hàng đợi duyệt, trả mọi trạng thái.
  - `109` `DELETE /rooms/{roomId}` (`landlord`) — xoá mềm phòng, `409 ROOM HAS ACTIVE CONTRACT` khi phòng còn hợp đồng hiệu lực.
  - `110` `POST /match-requests/{matchRequestId}/withdraw-request` (`tenant`) — người gửi rút yêu cầu.
  - `MatchRequest.status` thêm giá trị `withdrawn`. Dùng trạng thái chứ không dùng xoá bản ghi: một yêu cầu đã rút là dữ liệu đáng giữ, và nó phân biệt được với một yêu cầu bị từ chối.
- **Reason:** Cả ba đều là chỗ người dùng thật sẽ bị chặn, không phải thiếu tiện nghi. Riêng (1) là lỗi nghiêm trọng nhất vì nó biến một bước duyệt có kiểm soát thành một cú bấm mù. Với (3), `withdrawn` là lựa chọn nhẹ nhất: enum Postgres không cần cột mới, còn `is_deleted` sẽ phải thêm cột và vẫn không phân biệt được "đã rút" với "đã xoá". Action route thay vì `DELETE` vì `withdraw-request` phải nằm cạnh `accept-request`/`reject-request` theo quy ước đặt tên của D48, và bên nhận vẫn dùng `reject-request` nên `409 NOT REQUESTER` là đủ để phân biệt.
- **Consequence:** Hợp đồng lên **109 endpoint trong 24 nhóm**. Số nhóm không đổi vì 3 endpoint rơi vào 3 nhóm đã có. Đánh số chỉ nối thêm, không dịch mã, nên §29, mọi cross-reference `API N` và citation UI giữ nguyên; bảng §29 chuyển từ 3 lên 6 endpoint thuần mới. **Thay đổi xuyên phase:** `phase-1/discovery/database-design.md` §2.20 phải thêm `withdrawn` vào enum `MatchRequest.status`; migration chỉ là `ALTER TYPE`, không thêm cột. Một hệ quả chưa giải quyết: UNIQUE `(requester_id, target_id)` khiến một cặp đã `withdrawn` không gửi lại được, cần PM chốt có bỏ ràng buộc toàn bảng hay đổi thành unique partial trên `status = 'pending'` hay không.
- **Not decided / left to migration:** UNIQUE `(requester_id, target_id)` và hệ quả gửi lại. Chưa chốt.
- **Cross-ref:** `api-spec.md` §6, §7, §10, §27.5, §27.6, §27.9, §28.9, §29; `api-conventions.md` §soft-delete; `phase-1/discovery/database-design.md` §2.20; `P2-06-api-spec.md`.