...
**Output:** `openapi.yaml` trong `pbl6-backend` (handoff được generate ở repo `pbl6-backend`, ngoài workspace này) · design input: `discovery/api-spec.md` (109 endpoints, 24 groups)
...
109 endpoints trong 24 nhóm domain. Số liệu này đã đối chiếu trực tiếp với 24 bảng domain và 109 endpoint block trong `api-spec.md` §27:

1. Auth (6)
2. User (9) — gồm 2 action `lock`, `unlock` tách khỏi PATCH
3. PushDevice (2)
4. Media (4)
5. Building (11) — tách theo vai trò: public `/buildings`, chủ nhà `/me/buildings`, admin `/admin/buildings` kèm chi tiết `/admin/buildings/{buildingId}`
6. Room (7) — gồm action `mark-ready` tách khỏi PATCH và `DELETE /rooms/{roomId}`
7. FavoriteRoom (3)
8. RoommateProfile (4)
9. MatchRequest (5) — có `withdraw-request` để người gửi rút yêu cầu
...
> **D50 (2026-09-26):** thêm 3 endpoint do rà soát phát hiện admin không đọc được tòa nhà `pending`, chủ nhà không xoá được một phòng, và người gửi match request không rút lại được: `GET /admin/buildings/{buildingId}` (108), `DELETE /rooms/{roomId}` (109), `POST /match-requests/{matchRequestId}/withdraw-request` (110). Mã 32 vẫn trống; mã cũ giữ nguyên. Mục 110 kéo theo một thay đổi enum ở Phase 1: `MatchRequest.status` thêm `withdrawn`.
...
- **Bulk meter-reading (`MeterReadingBatch`) — pending PM decision, D47.** PM đã quyết định giữ nguyên pending tại session hiện tại, chưa đưa vào v1. Baseline có `POST /buildings/{buildingId}/meter-reading-batches` cùng 3 màn hình draft đánh dấu `[MỚI]`, nhưng `requirement.md` chưa xác nhận và `api-conventions.md` §14 cấm bulk cho chốt chỉ số. v1 giữ nguyên 109 endpoint, chỉ có `POST /meter-readings` từng phòng; xem `api-spec.md` §28.14 và `decisions.md` D47. Đây là lý do chính khiến spec chưa thể freeze.
