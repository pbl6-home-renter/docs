...
| email | varchar | UNIQUE, NOT NULL | | `lan@example.com` |
...
| province | varchar | NOT NULL | tên Tỉnh/TP từ dropdown VN | `Thành phố Hồ Chí Minh` |
| ward | varchar | NOT NULL | tên Phường/Xã từ dropdown VN | `Phường Bến Nghé` |
| address | varchar | NOT NULL | địa chỉ chi tiết (số nhà, đường) | `12 Nguyễn Trãi` |
| latitude | decimal(9,6) | nullable | tự geocode từ address khi tạo building | `20.995100` |
| longitude | decimal(9,6) | nullable | tự geocode từ address khi tạo building | `105.816649` |
...
| note | text | nullable | bắt buộc khi `status='rejected'` | `Ảnh giấy tờ mờ` |
...
| floor_number | int | nullable | tầng (1-based) | `2` |
| owner_id | uuid | FK→User, **NOT NULL** | **D25 — bắt buộc, 1 chủ/phòng** | |
| name | varchar | NOT NULL | | `P.201` |
| description | text | nullable | **D21** | `Phòng trọ hiện đại, tiện nghi đầy đủ` |
...
| max_occupancy | int | nullable | **D21** hard filter ghép bạn | `3` |
...
| rent_price | decimal | NOT NULL | | `3500000` |
...
| tenant_name | varchar | nullable | dùng khi tenant không có account | `Trần Văn B` |
| tenant_phone | varchar | nullable | dùng khi tenant không có account | `0912345678` |
...
| deposit_amount | decimal | NOT NULL | số cọc khi ký HĐ; lịch sử trả cọc/điều chỉnh lưu trong Payment | `7000000` |
| monthly_rent | decimal | NOT NULL | | `3500000` |
| start_date | date | NOT NULL | | `2026-09-01` |
| end_date | date | NOT NULL | | `2027-08-31` |
| description | text | nullable | | `Đóng tiền trước ngày 5 hàng tháng…` |
...
| tenant_name | varchar | nullable | | `Nguyễn Thị C` |
| tenant_phone | varchar | nullable | | `0921234567` |
...
| name | varchar | NOT NULL | nhãn hiển thị | `Điện EVN`, `Wifi` |
...
| unit_price | decimal | nullable | `flat`/`per_head`/`per_area_m2`/`per_vehicle`: đ/unit; `tiered`: NULL (dùng steps) | `80000` |
...
| due_days | smallint | NOT NULL default 5 | ngày trong tháng chốt hóa đơn | `5` |
| remind_days | smallint | NOT NULL default 3 | ngày trong tháng nhắc nộp tiền | `3` |
...
| previous_reading | decimal | NOT NULL | kỳ đầu = baseline chỉ số lúc bàn giao §5; kỳ sau = current_reading kỳ trước | `1200` |
...
| rent_amount | decimal | NOT NULL | tiền phòng **prorated** theo §8 (`monthly_rent × ratio`); denormalized cho Dashboard (D20) | `3500000` |
...
| title | varchar | nullable | tên group chat (optional, cho building chat hoặc future use) | `Chat tòa nhà` |
