...
Mọi entity ở mục 2.1 → 2.21 dưới đây được hiểu là **kế thừa 4 field trên** (không lặp lại trong từng bảng để bảng gọn). Các unique/partial index có điều kiện trạng thái (vd 1 hợp đồng active/phòng) đều kèm thêm `AND is_deleted = false`.
...
| role | enum (`landlord`,`tenant`,`admin`) | NOT NULL | **D13** — exactly one role per user | `landlord` |
...
| settings | jsonb | nullable | cấu hình invoice/contract mặc định | `{"invoice_due_day":5, "remind_after_due_days":3}` |
...
| tenant_user_id | uuid | FK→User, **nullable** | nullable để hỗ trợ tenant chưa có account (**D29**); nếu null thì lưu tên/SĐT trực tiếp trên contract | `NULL` |
| tenant_name | varchar | nullable; required when `tenant_user_id IS NULL` | Tenant name for an accountless tenant (D29) | `Trần Văn B` |
| tenant_phone | varchar | nullable; required when `tenant_user_id IS NULL` | Tenant phone for an accountless tenant (D29) | `0912345678` |
...
| scope | enum (`landlord`,`building`,`room`) | NOT NULL | cấp áp dụng; `landlord` = mặc định cả nhà (trỏ từ `LandlordProfile.electricity_policy_id`/`LandlordProfile.water_policy_id`); scope thực tế thể hiện ở cột ID trên bảng chủ (profile/tòa/phòng) | `room` |
...
| report_by_user_id | uuid | FK→User, **nullable** | null nếu landlord tự tạo thay tenant passive (**D29**) | `NULL` |
| status | enum (`open`,`in_progress`,`resolved`) | NOT NULL default `open` | | `open` |
| title | varchar | NOT NULL | | `Đèn cháy` |
| description | text | NOT NULL | | `Bóng đèn phòng ngủ cháy 2 bóng` |
...
LandlordProfile N--0..1 UtilityRatePolicy  (electricity_policy_id)
LandlordProfile N--0..1 UtilityRatePolicy  (water_policy_id)
...
User 1--N MatchRequest (requester_user_id / target_user_id)
...
| ContractMember.cccd_ocr_data + Media `cccd_front`/`cccd_back` | D17 |
| User.role is a single enum value | D13 |
...
| Building omits `property_type` per supplied revision; short-term remains outside MVP | D9 |
...
- **Không có bảng `Role` hay `Permission` riêng.** Quyền được suy ra trực tiếp từ 2 lớp kiểm tra ở tầng application (service/guard), không phải từ dữ liệu quan hệ:
  1. **Role check** — user có role cần thiết không (`landlord` / `tenant` / `admin`), compare `User.role` to the required single value (**D13**).
...
- Backend dùng decorator kiểu `@Roles('landlord')` trên từng endpoint (`user.role === 'landlord'`) + 1 hàm `assertOwnership(resource, user)` gọi thêm trong service layer khi cần. Không cần bảng ánh xạ role↔permission vì tập hành động cố định, không thay đổi runtime.
...
- Nếu sau này cần permission tùy biến (vd. landlord có nhân viên phụ, multi-org), có thể mở rộng thêm bảng `Role`/`Permission` ở Phase 1+ and explicitly migrate `User.role` if multiple roles become necessary.
