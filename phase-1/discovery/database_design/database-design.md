...
1. **Role không dùng RBAC (không có bảng Role/Permission riêng).**User.role là  **1 giá trị đơn** enum tĩnh: landlord | tenant | admin (đổi từ mảng — mỗi user chỉ giữ đúng 1 role tại một thời điểm; muốn đổi vai trò phải cập nhật lại field này, không cộng dồn). Kiểm tra quyền = so khớp role trong JWT + kiểm tra sở hữu (ownership check) ở tầng service, không cần bảng trung gian.  
...
6. **Schema giữ mở cho short-term (Phase 1+)** — trước đây qua field Building.property_type; field này đã bị bỏ theo yêu cầu v4, việc mở rộng loại hình cho thuê ngắn hạn sẽ cần thêm lại field tương đương ở Phase 1+ khi triển khai thực tế (D9).  
7. **Mọi luồng nghiệp vụ phải chạy được với ** **tenant_user_id = NULL** trên các bản ghi liên quan tenant thụ động (D29) — xem mục 5.  
8. **Base Entity thống nhất** — mọi bảng đều có id, created_at, updated_at, is_deleted (soft-delete dạng boolean), cập nhật updated_at tự động bằng trigger — xem mục 2.0.  
...
   
 | is_deleted | boolean | NOT NULL, default false | quy ước soft-delete: false = còn tồn tại. Query mặc định luôn thêm WHERE is_deleted = false. Với bảng mang tính chứng từ/audit (invoice, payment, meter_reading...) cột này gần như không dùng tới trong nghiệp vụ (không "xoá" hoá đơn) nhưng vẫn giữ để đồng nhất schema. |  
Mọi entity ở mục 2.1 → 2.19 dưới đây được hiểu là **kế thừa 4 field trên** (không lặp lại trong từng bảng để bảng gọn). Các unique/partial index có điều kiện trạng thái (vd 1 hợp đồng active/phòng) đều kèm thêm AND is_deleted = false.  
   
 **2.1 User**  
   
 | | | | |  
...
   
 | role | enum (landlord,tenant,admin) | NOT NULL | **v4** — 1 giá trị duy nhất/user (đổi từ mảng theo D13 gốc); 1 người muốn vừa cho thuê vừa đi thuê cần 2 tài khoản riêng |  
...
   
 | electricity_rate | decimal | NOT NULL | **D16** — set 1 lần, áp mọi tòa |  
   
 | water_rate | decimal | NOT NULL | **D16** |  
   
 | settings | jsonb | nullable | cấu hình invoice/contract mặc định |  
**2.3 Building**  
   
 | | | | |  
   
...
 | id | uuid | PK |   |  
   
 | owner_id | uuid | FK→User, **nullable** | building  **không bắt buộc** có chủ (  **D25**) |  
   
...
   
 | electricity_rate | decimal | nullable | override cấp tòa (**D16**) |  
   
 | water_rate | decimal | nullable | override cấp tòa (**D16**) |  
...
 | name | varchar | NOT NULL |   |  
   
 | area | decimal | nullable | diện tích (m²) |  
   
...
   
 | electricity_rate_override | decimal | nullable | **D16** — thứ tự đọc: room → building → landlord |  
   
...
   
 | tenant_user_id | uuid | FK→User, **nullable** | nullable để hỗ trợ tenant chưa có account ( **D29**); nếu null thì lưu tên/SĐT trực tiếp trên contract |  
   
...
   
 **Index quan trọng:** partial unique index (room_id) WHERE status = 'active' AND is_deleted = false — chỉ 1 hợp đồng active/phòng.  
   
...
 | room_id | uuid | FK→Room, NOT NULL |   |  
   
 | name | enum (electricity,water) | NOT NULL | **v4** — phân biệt công tơ điện/nước trong cùng kỳ |  
   
...
   
 **Index:** UNIQUE (room_id, period, name).  
   
 **2.9 Invoice**  
   
 | | | | |  
...
   
 | note | text | nullable |   |  
**Index:** partial unique (contract_id, period) WHERE status != 'void' — cho phép phát hành lại hoá đơn cho cùng kỳ sau khi void (thay cho cột voided_invoice_id đã bỏ ở v4; việc "hoá đơn nào thay thế hoá đơn nào" tra theo contract_id + period + issued_at).  
   
 **2.10 Payment**  
   
 | | | | |  
...
   
 | report_by_user_id | uuid | FK→User, **nullable** | null nếu landlord tự tạo thay tenant passive ( **D29**) |  
   
...
*Ảnh sự cố: * * *Media(owner_type='issue_report', purpose='issue_photo')* * * — không thêm cột * *photos* * riêng dù ERD có gợi ý, để tránh rải cột file (nguyên tắc #9).*  
   
...
   
 | **A. 1 bảng polymorphic**media(owner_type, owner_id, purpose, ...) — mô hình dùng bởi Rails Active Storage, Django GenericForeignKey, kiểu Instagram/Airbnb | 1 chỗ duy nhất quản lý file (upload, xoá, CDN, resize); thêm loại entity mới không cần bảng mới; dễ audit ai upload | owner_id **không có FK thật** (1 cột không FK được nhiều bảng) → toàn vẹn tham chiếu phải kiểm tra ở tầng service |  
   
 | **B. Bảng join riêng từng entity** (RoomPhoto, BuildingPhoto, ContractDocument, IssuePhoto, ChatAttachment,...) | FK thật, DB tự đảm bảo toàn vẹn tham chiếu | 6-8 bảng gần như trùng cấu trúc (id, url, uploaded_by, timestamps); thêm entity mới lại phải thêm bảng + migration mới |  
...
**v4 — bộ field rút gọn theo ERD** (bỏ thumbnail_url, mime_type, width, height, display_order, is_cover so với v3; đổi uploaded_by_user_id → user_id):  
   
 | | | | |  
   
 |-|-|-|-|  
...
   
 | user_id | uuid | FK→User, nullable | người upload |  
**Index:** (owner_type, owner_id) — query nhanh toàn bộ media của 1 entity. Partial unique (owner_type, owner_id, purpose) WHERE purpose='cover_photo' AND is_deleted=false — thay cho cờ is_cover của v3, đảm bảo mỗi entity chỉ 1 ảnh bìa. Thứ tự hiển thị gallery (trước đây display_order) mặc định lấy theo created_at ASC; nếu cần sắp xếp tuỳ ý, cân nhắc thêm lại cột này ở Phase 1+.  
   
...
   
 User 1--N Building (owner_id, nullable)  
   
 Building 1--N Floor  
   
 Building 1--N Room  
...
   
 | User.role là 1 giá trị đơn (đổi từ mảng ở v3/D13 theo yêu cầu cập nhật — không còn multi-role/tài khoản) | D13 (điều chỉnh ở v4) |  
   
...
   
 | Contract.tenant_user_id nullable, ContractMember.user_id nullable, IssueReport.report_by_user_id nullable | D29 (zero tenant login) |  
   
 | Building.property_type đã bỏ ở v4 — mở rộng short-term (Phase 1+) sẽ cần thêm lại field tương đương | D9 (điều chỉnh ở v4) |  
   
 | Building.latitude/longitude | yêu cầu định vị bản đồ |  
   
 | Bảng Media polymorphic, bộ field rút gọn theo ERD (thay mọi cột *_url/photos) | yêu cầu tránh rải url file |  
   
 | Base Entity (id/created_at/updated_at/is_deleted) trên mọi bảng | yêu cầu chuẩn hoá audit/soft-delete |  
![](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAnEAAAACCAYAAAA3pIp+AAAABmJLR0QA/wD/AP+gvaeTAAAACXBIWXMAAA7EAAAOxAGVKw4bAAAANElEQVR4nO3OQQmAABRAsSeYxKQ/i8mMIR7ECt5E2BJsmZmt2gMA4C+Otbqr8+sJAACvXQ85QAYaeQO2UAAAAABJRU5ErkJggg==)  
   
...
- **Không có bảng ** ** **Role** ** ** hay ** ** **Permission** ** ** riêng.** Quyền được suy ra trực tiếp từ 2 lớp kiểm tra ở tầng application (service/guard), không phải từ dữ liệu quan hệ:  
  1. **Role check** — user có đúng role cần thiết không (landlord / tenant / admin), lấy từ User.role (1 giá trị đơn, so sánh bằng —  **v4**, khác v3 dùng mảng includes).  
  2. **Ownership check** — nếu resource có owner_id/issued_by_user_id/tenant_user_id/user_id thì phải khớp với request.user.id, trừ khi role = admin.  
- JWT payload: { sub: user_id, role: "landlord" } — không nhúng permission chi tiết, không còn là mảng.  
- Backend dùng decorator kiểu @Roles('landlord') trên từng endpoint (so sánh user.role === 'landlord') + 1 hàm assertOwnership(resource, user) gọi thêm trong service layer khi cần. Không cần bảng ánh xạ role↔permission vì tập hành động cố định, không thay đổi runtime.  
- **Lưu ý v4:** vì mỗi user chỉ có 1 role, một cá nhân vừa muốn cho thuê vừa muốn đi thuê phòng (trường hợp D13 gốc có tính đến) cần tạo 2 tài khoản (2 số điện thoại/email) khác nhau — không còn gộp chung 1 User.  
...
   
 | Contract | Đọc | ✅ (owner) | ✅ (chỉ HĐ mình là tenant_user_id/ContractMember) | ✅ |   |  
   
...
- Nếu sau này cần permission tùy biến (vd. landlord có nhân viên phụ, multi-org) hoặc cần lại multi-role/user, có thể mở rộng User.role trở lại thành mảng hoặc thêm bảng Role/Permission ở Phase 1+ mà không phá vỡ phần còn lại của schema.  
