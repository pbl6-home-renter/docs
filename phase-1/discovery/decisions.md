...
| D39 | 2026-09-26 | **Đơn giá 1 bộ/scope (bỏ versioning) + app không cưỡng chế thu tiền + lịch sử nằm ở snapshot** — `RatePolicy` gộp `UtilityRatePolicy`+`RecurringFee` thành `rates` jsonb, chỉ UPDATE, scope `building`\|`room` (bỏ cấp landlord + 2 con trỏ `*_policy_id`); `Invoice.invoice_status` = `pending`\|`issued`\|`void`, bỏ `partially_paid`/`paid`/`overdue`/`cancel` + bỏ `due_date` (trạng thái thu suy ra từ Σ `Payment` success); `Invoice.breakdown` gộp 1 mảng snapshot đủ `rate_kind`/`quantity`/`unit`/`service_start`/`service_end`; `BillingSetting` → `payment_due_day`/`remind_day` advisory; thêm tài khoản nhận tiền vào `LandlordProfile` để sinh QR sandbox; `Payment` không `collected_at` (dùng `created_at`), `mock`/`cash` chính + `vietqr` sandbox, đối soát theo **mã hóa đơn** không theo amount; auto-reconciliation chuyển sang Stretch | ✅ Accepted |
...
- **Decision:**
  ...
  - **Kê khai xe tại HĐ:** thêm `Contract.vehicle_count` (int, nullable) — khai lúc tạo/cập nhật hợp đồng (cạnh CCCD/tiền cọc). Phí `per_vehicle` tính theo số này, snapshot vào `breakdown[]` (D39); đổi giữa kỳ → cập nhật HĐ, hóa đơn chưa phát hành dùng số mới, đã phát hành giữ snapshot cũ. `NULL` khi có phí `per_vehicle` hoạt động → block (`VEHICLE_COUNT_MISSING`, EC8). (Bảng `VehicleRegister` vẫn ngoài MVP như D30.)
...
## D39 — Đơn giá 1 bộ/scope (bỏ versioning) + app không cưỡng chế thu tiền + lịch sử nằm ở snapshot

> **Ghi chú:** D39 **sửa/thu hẹp** D30 (versioning `effective_*`, cấp `landlord`, 2 bảng riêng), D32 (con trỏ `*_policy_id` nullable), D33 (enum `partially_paid` + quét `overdue`), D36 (áp dụng cho `RecurringFee` → giờ gộp), D37 (`due_date = issued_at + N` làm gốc quét `overdue`). **Các ID cũ giữ nguyên, không đổi nội dung lịch sử.**

- **Context:** Rà lại chuỗi tính phí + thu tiền trước khi chốt scope, phát hiện 4 điểm tốn độ phức tạp mà **không đổi giá trị người dùng**, và 1 rủi ro nghiệp vụ thật:
  1. **Versioning giá là chi phí, không phải lợi ích.** D30 thêm `effective_from/to` + sinh bản sao để "giữ lịch sử giá" — nhưng hóa đơn đã snapshot giá tại thời điểm lập (D30/D37), nên lịch sử **đã nằm sẵn trong `Invoice`**. Bảng version chỉ thêm: chọn nhầm policy, orphan policy, UI phải có màn "lịch sử giá" mà chủ trọ lớn tuổi (đối tượng chính, D31) không bao giờ xem.
  2. **Cấp `landlord` là cầu nối không cần thiết.** Chuỗi `room → building → landlord` + 2 con trỏ trên cả 3 bảng = 6 cột, 3 nơi phải đồng bộ, 3 chỗ có thể lệch — để làm *một* việc: phòng không có giá riêng thì lấy giá tòa.
  3. **App không có quyền cưỡng chế thu tiền.** `overdue` + `due_date` giả định hệ thống *quyết định* hóa đơn hết hạn và chặn thao tác. Thực tế chủ trọ VN thu tiền mặt/tài khoản ngoài app, hóa đơn app chỉ là **bản ghi để đối soát** → `overdue` sẽ là dữ liệu sai lệch, và chặn thao tác sẽ chặn cả việc *ghi nhận* tiền đã thu.
  4. **`collected_at` trùng `created_at`.** Mốc "chủ trọ bấm đã thu" và "dòng Payment được tạo" là **cùng một sự kiện** ở MVP (ghi tiền thật cũng nhập tay qua nút "đã thu"). Thêm cột thứ hai chỉ tạo điều kiện lệch dữ liệu.
  5. **Đối soát theo amount là sai.** Khách trả thiếu/dư hoặc trả gộp nhiều hóa đơn → khớp tiền nhầm. Mã hóa đơn (`Invoice.code`) nhúng trong nội dung chuyển khoản là định danh duy nhất đáng tin.
- **Decision:**
  - **`RatePolicy` — 1 bộ/scope, chỉ UPDATE, không versioning:** gộp `UtilityRatePolicy` + `RecurringFee` vào `rates` jsonb (`type`,`name`,`rate_kind`,`unit_price`,`steps`,`billing_unit`; mỗi `type` tối đa 1 mục; validate ở service layer). Bỏ `effective_from/to`, không tạo bản sao, không xóa mềm. `is_active = false` = tắt cả bộ → kế thừa cấp trên (giữ nguyên ngữ nghĩa D36). **Đổi giá = UPDATE tại chỗ, tính bằng bộ rate đang chạy tại thời điểm tính hóa đơn** → hóa đơn **chưa phát hành** của kỳ đó dùng giá mới; hóa đơn **đã phát hành** giữ nguyên snapshot cũ, không bao giờ sửa lại.
  - **Bỏ cấp `landlord`:** `scope` chỉ còn `building | room`; kế thừa `room → building`. Xóa `electricity_policy_id`/`water_policy_id` khỏi `Room`/`Building`/`LandlordProfile` — `RatePolicy` tự khai scope/đích, link 1 chiều. Bộ "mặc định của chủ trọ" = bộ của từng tòa (UI có thể hỗ trợ nhân bản).
  - **`Invoice` — app không cưỡng chế:** `invoice_status` chỉ còn `pending` | `issued` | `void` (xoá `partially_paid`, `paid`, `overdue`; `cancel` → `void`). **Không lưu trạng thái thu tiền** — UI suy ra `chưa thu / thu một phần / thu đủ / thu dư` từ Σ `Payment` success so với `total_amount`. Bỏ `due_date`; giữ `voided_invoice_id` (new → old) và partial unique `(contract_id, period) WHERE invoice_status IN ('pending','issued') AND is_deleted = false`.
  - **`Invoice.breakdown` — một mảng duy nhất:** thay `utility_breakdown` + `fees_breakdown`; mỗi dòng đủ `type`,`name`,`rate_kind`,`unit_price`,`quantity`,`unit`,`service_start`,`service_end`,`prorate_ratio`,`amount` (+`tier_detail` cho bậc thang). `service_start`/`service_end` = khoảng dịch vụ **thực tế** (khác `Invoice.period` khi HĐ vào/ra giữa tháng) → giải thích được "khoản này thu cho tháng nào". Invariant: `rent_amount + Σ(breakdown) + Σ(other_fees) == total_amount`, `other_fees` âm được, `total_amount ≥ 0`.
  - **`BillingSetting` thành gợi ý:** `due_days`/`remind_days` → `payment_due_day`/`remind_day` (int trong tháng), chỉ để hiển thị "thường thu ngày 5"; **không** sinh `overdue`, **không** chặn thao tác. Resolve `room → building`, không có cấp `landlord`.
  - **Tài khoản nhận tiền:** thêm `LandlordProfile.bank_code` / `account_number` / `account_name` (nullable) — landlord khai 1 lần để sinh **QR động nhúng số tiền + `Invoice.code`**; dữ liệu MVP là sandbox/mock, không xác thực ngân hàng thật.
  - **`Payment`:** **không `collected_at`** (dùng `created_at`); `method` giữ nguyên `vietqr|vnpay|momo|cash|mock`, `mock`/`cash` là kênh chính, `vietqr` sandbox tầng 1, `vnpay`/`momo` Stretch. Đối soát theo **`Invoice.code`**, không theo amount. `Payment success` trỏ tới hóa đơn `void` → `needs_review` cho chủ trọ xử lý. Thu dư **không** tự hoàn/bù trừ — hiển thị nhắc ở tab History.
  - **Scope thu tiền:** MVP = ghi nhận thủ công (`mock`/`cash`) + QR sandbox. **Auto-reconciliation từ nội dung chuyển QR/webhook sang Stretch** (cần merchant/đối tác thật + idempotency + bảo mật chữ ký) — không tự đánh dấu nợ chỉ vì có tiền vào tài khoản.
- **Alternatives rejected:** giữ versioning (chi phí cấu hình, lịch sử đã có trong snapshot); giữ cấp `landlord` (6 cột + 3 nơi đồng bộ cho 1 nhu cầu); giữ `overdue` + `due_date` + `partially_paid` (dữ liệu sai lệch với thực tế thu tiền ngoài app, và chặn thao tác khiến không ghi được tiền đã thu); thêm `collected_at` (trùng `created_at`); đối soát theo `amount` (sai khi trả thiếu/dư/gộp); auto-reconciliation trong MVP (chưa có đối tác thật, rủi ro đánh dấu sai nợ).
- **Impact:** `database-design.md` §2.2 (`LandlordProfile` + tài khoản nhận tiền), §2.3/§2.4 (bỏ con trỏ), §2.9 (`RatePolicy`), §2.10 (`BillingSetting`), §2.12 (`Invoice` + `breakdown`), §2.13 (`Payment`), §3 ERD, §4 traceability, §5 permission matrix, §6 open questions, §7 migration notes; `utility-billing-calculations.md` §0/§1/§4/§5/§6/§7/§9/§10/§11 (EC3/4/6/9/15)/§12/§13; `user_flow/landlord_user_flow.md` + `tenant-user-flow.md` (tài khoản nhận tiền, QR, trạng thái thu, bỏ nhắc quá hạn); `requirement.md`, `PROJECT_PLAN.md`, `feature-list.md`, `product-sketch.md` (auto-reconciliation → Stretch).
- **Status:** ✅ Accepted 2026-09-26 (user/PM duyệt).